...
@click.option(
    ...
    "-t",
    "device_type",
    type=click.Choice(["all", "input", "loopback"]),
    default="all",
...
def _diagnose_loopback() -> None:
    ...
    try:
        ...
        if wasapi is None:
            ...
            click.echo(
                f"    WASAPI host API: OK (default_output={wasapi['default_output_device']})"
            )
            ...
            if not out_devs:
                click.echo(
                    "    WARNING: no WASAPI output devices — loopback impossible via sounddevice"
                )
    ...
    try:
        ...
        try:
            ...
            click.echo(
                f"    WASAPI available: YES (default_output={wasapi_info['defaultOutputDevice']})"
            )
            loopback_devs = [
            ...
            for d in loopback_devs:
                click.echo(
                    f"      [{d['index']}] {d['name']}  ch={d['maxInputChannels']}  rate={d['defaultSampleRate']}"
                )
            if not loopback_devs:
                click.echo(
                    "    WARNING: no loopback devices — pyaudiowpatch won't work on this machine"
                )
        except Exception as exc:
