...
def test_normal_mode_keeps_mixer_info_event() -> None:
    event_dict = {"level": "info", "event": "mixer.started"}

    assert _filter_normal_mode_events(logging.getLogger("test"), "info", event_dict) == event_dict


def test_normal_mode_keeps_vad_chunker_info_event() -> None:
    event_dict = {"level": "info", "event": "vad_chunker.emit"}
