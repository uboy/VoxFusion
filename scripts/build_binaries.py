...
def _collect_all_packages(*, backends: set[str]) -> list[str]:
    ...
    if _is_installed("pyannote.audio"):
        candidates.append("pyannote.audio")
        print("[deps] --collect-all pyannote.audio  (ML diarization / speaker detect)")
    else:
        print("[deps] WARNING: pyannote.audio not installed — ML diarization and speaker detect won't work")
...
def _ffmpeg_data_entries() -> list[str]:
    ...
    if ffmpeg and ffmpeg.exists():
        ...
        try:
            ...
            ffmpeg = None
            print(f"[ffmpeg] WARNING: failed to prepare portable FFmpeg: {exc}")
    else:
        ffmpeg = None
...
def _pyannote_data_entries() -> list[str]:
    ...
    if not config_path.exists():
        print("[deps] WARNING: pyannote telemetry config.yaml not found, ML diarization bundle may be incomplete")
        return []
...
def _package_file_data_entry(package_name: str, filename: str) -> str | None:
    ...
    if not file_path.exists():
        print(f"[deps] WARNING: {package_name}/{filename} not found, frozen bundle may be incomplete")
        return None
...
def _default_data_entries() -> list[str]:
    ...
    for package_name in ("lightning_fabric", "pytorch_lightning"):
        version_entry = _package_file_data_entry(package_name, "version.info")
